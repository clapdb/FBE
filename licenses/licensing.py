...
def main(path: str, inplace: bool):
        ...
        for root, dirs, files in os.walk(abs_path):
            for file in files:
                ...
                if not is_excluded(abs_file):
                    file_license(abs_file, inplace)

def file_license(file: str, inplace: bool):
    # do file licensing
    if should_license(file):
        licensing(file, inplace)

def licensing(file: str, inplace: bool):
    if inplace:
...
def in_place_license(file: str, position: tuple[int, int]):
    ...
    if begin > end or begin > length or end > length:
        ...
        return
    # make clang format happy, not leading space.
    if begin == 1:
        begin = 0
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--inplace', help='update license inplace', action="store_true")
    args = parser.parse_args()
    main(args.file, args.inplace)
