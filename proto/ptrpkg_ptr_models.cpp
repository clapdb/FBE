...
#include "ptrpkg_ptr_models.h"

namespace FBE {

FieldModelPtr_ptrpkg_Line::FieldModelPtr_ptrpkg_Line(FBEBuffer& buffer, size_t offset) noexcept : _buffer(buffer), _offset(offset)
{}

FieldModelPtr_ptrpkg_Line::~FieldModelPtr_ptrpkg_Line()
{
    if (ptr) delete ptr;
}

size_t FieldModelPtr_ptrpkg_Line::fbe_extra() const noexcept
{
    if (!ptr) return 0;
...
bool FieldModelPtr_ptrpkg_Line::verify() const noexcept
{
    if (!ptr) return true;
...
bool FieldModelPtr_ptrpkg_Line::has_value() const noexcept
{
    if ((_buffer.offset() + fbe_offset() + fbe_size()) > _buffer.size())
        return false;
...
size_t FieldModelPtr_ptrpkg_Line::get_begin() const noexcept
{
    if (!has_value())
        return 0;
...
void FieldModelPtr_ptrpkg_Line::get_end(size_t fbe_begin) const noexcept
{
    _buffer.unshift(fbe_begin);
}

void FieldModelPtr_ptrpkg_Line::get(::ptrpkg::Line** fbe_value, std::pmr::memory_resource* resource) noexcept
{
...
size_t FieldModelPtr_ptrpkg_Line::set_begin(bool has_value)
{
...
void FieldModelPtr_ptrpkg_Line::set_end(size_t fbe_begin)
{
    _buffer.unshift(fbe_begin);
}

void FieldModelPtr_ptrpkg_Line::set(const ::ptrpkg::Line* fbe_value, std::pmr::memory_resource* resource) noexcept
{
...
size_t FieldModel_ptrpkg_Line::fbe_extra() const noexcept
{
    if ((_buffer.offset() + fbe_offset() + fbe_size()) > _buffer.size())
        return 0;
...
bool FieldModel_ptrpkg_Line::verify(bool fbe_verify_type) const noexcept
{
    if ((_buffer.offset() + fbe_offset() + fbe_size()) > _buffer.size())
        return true;
...
size_t FieldModel_ptrpkg_Line::get_begin() const noexcept
{
    if ((_buffer.offset() + fbe_offset() + fbe_size()) > _buffer.size())
        return 0;
...
void FieldModel_ptrpkg_Line::get_end(size_t fbe_begin) const noexcept
{
    _buffer.unshift(fbe_begin);
}

void FieldModel_ptrpkg_Line::get(::FBE::Base& fbe_value, std::pmr::memory_resource* resource) noexcept
{
    size_t fbe_begin = get_begin();
    if (fbe_begin == 0)
        return;
...
void FieldModel_ptrpkg_Line::get_fields([[maybe_unused]] ::FBE::Base& base_fbe_value, [[maybe_unused]] size_t fbe_struct_size, std::pmr::memory_resource* resource) noexcept
{
    ...
    if ((fbe_current_size + value.fbe_size()) <= fbe_struct_size)
        ...
        fbe_value.value = ::variants_ptr::Value();
    fbe_current_size += value.fbe_size();

    if ((fbe_current_size + value_ptr.fbe_size()) <= fbe_struct_size)
        {
            value_ptr.get(&fbe_value.value_ptr, nullptr);
        }
    else
...
size_t FieldModel_ptrpkg_Line::set_begin()
{
    assert(((_buffer.offset() + fbe_offset() + fbe_size()) <= _buffer.size()) && "Model is broken!");
    if ((_buffer.offset() + fbe_offset() + fbe_size()) > _buffer.size())
        return 0;
...
void FieldModel_ptrpkg_Line::set_end(size_t fbe_begin)
{
    _buffer.unshift(fbe_begin);
}

void FieldModel_ptrpkg_Line::set(const ::FBE::Base& fbe_value, std::pmr::memory_resource* resource) noexcept
{
...
void FieldModel_ptrpkg_Line::set_fields([[maybe_unused]] const ::FBE::Base& base_fbe_value, std::pmr::memory_resource* resource) noexcept
{
...
size_t LineModel::serialize(const ::ptrpkg::Line& value, std::pmr::memory_resource* resource)
{
...
size_t LineModel::deserialize(::ptrpkg::Line& value, std::pmr::memory_resource* resource) noexcept
{
    if ((this->buffer().offset() + model.fbe_offset() - 4) > this->buffer().size())
        return 0;
