...
public class ValueFinalModel: Model {
    private let _model: FinalModelValue

    public override init(buffer: Buffer = Buffer()) {
        _model = FinalModelValue(buffer: buffer, offset: 8)
    ...
    static let fbeTypeConst: Int = FinalModelValue.fbeTypeConst
    ...
    public func serialize(value: Value) throws -> Int {
    ...
    public func deserialize() -> Value { var value = Value(); _ = deserialize(value: &value); return value }

    public func deserialize(value: inout Value) -> Int {
        if (buffer.offset + _model.fbeOffset) > buffer.size {
            assertionFailure("Model is broken!")
            return 0
        }
