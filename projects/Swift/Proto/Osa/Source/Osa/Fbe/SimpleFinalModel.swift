...
public class SimpleFinalModel: Model {
    private let _model: FinalModelSimple

    public override init(buffer: Buffer = Buffer()) {
        _model = FinalModelSimple(buffer: buffer, offset: 8)
    ...
    static let fbeTypeConst: Int = FinalModelSimple.fbeTypeConst
    ...
    public func serialize(value: Simple) throws -> Int {
    ...
    public func deserialize() -> Simple { var value = Simple(); _ = deserialize(value: &value); return value }

    public func deserialize(value: inout Simple) -> Int {
        if (buffer.offset + _model.fbeOffset) > buffer.size {
            assertionFailure("Model is broken!")
            return 0
        }
