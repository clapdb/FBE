...
public class ExtraFinalModel: Model {
    private let _model: FinalModelExtra

    public override init(buffer: Buffer = Buffer()) {
        _model = FinalModelExtra(buffer: buffer, offset: 8)
    ...
    static let fbeTypeConst: Int = FinalModelExtra.fbeTypeConst
    ...
    public func serialize(value: Extra) throws -> Int {
    ...
    public func deserialize() -> Extra { var value = Extra(); _ = deserialize(value: &value); return value }

    public func deserialize(value: inout Extra) -> Int {
        if (buffer.offset + _model.fbeOffset) > buffer.size {
            assertionFailure("Model is broken!")
            return 0
        }
