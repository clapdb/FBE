...
// Fast Binary Encoding Char->String map final model
type FinalModelMapCharString struct {
    // Final model buffer
    buffer *fbe.Buffer
    ...
    // Map key final model
    modelKey *fbe.FinalModelChar
    // Map value final model
    modelValue *fbe.FinalModelString
}

// Create a new Char->String map final model
func NewFinalModelMapCharString(buffer *fbe.Buffer, offset int) *FinalModelMapCharString {
    fbeResult := FinalModelMapCharString{buffer: buffer, offset: offset}
    fbeResult.modelKey = fbe.NewFinalModelChar(buffer, offset)
    fbeResult.modelValue = fbe.NewFinalModelString(buffer, offset)
    return &fbeResult
}
...
func (fm *FinalModelMapCharString) FBEOffset() int { return fm.offset }
// Set the final offset
func (fm *FinalModelMapCharString) SetFBEOffset(value int) { fm.offset = value }

// Shift the current final offset
func (fm *FinalModelMapCharString) FBEShift(size int) { fm.offset += size }
// Unshift the current final offset
func (fm *FinalModelMapCharString) FBEUnshift(size int) { fm.offset -= size }

// Check if the map is valid
func (fm *FinalModelMapCharString) Verify() int {
