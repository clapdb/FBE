...
type FieldModelValue struct {
    ...
    V *FieldModelV
    Vo *FieldModelOptionalV
    Vo2 *FieldModelOptionalV
}
...
func NewFieldModelValue(buffer *fbe.Buffer, offset int) *FieldModelValue {
    ...
    fbeResult.Vo = NewFieldModelOptionalV(buffer, fbeResult.V.FBEOffset() + fbeResult.V.FBESize())
    fbeResult.Vo2 = NewFieldModelOptionalV(buffer, fbeResult.Vo.FBEOffset() + fbeResult.Vo.FBESize())
    return &fbeResult
}
...
func (fm *FieldModelValue) VerifyFields(fbeStructSize int) bool {
    ...
    if (fbeCurrentSize + fm.Vo.FBESize()) > fbeStructSize {
        return true
    }
    if !fm.Vo.Verify() {
        return false
    }
    fbeCurrentSize += fm.Vo.FBESize()

    if (fbeCurrentSize + fm.Vo2.FBESize()) > fbeStructSize {
        return true
    }
    if !fm.Vo2.Verify() {
        return false
    }
    fbeCurrentSize += fm.Vo2.FBESize()
...
func (fm *FieldModelValue) SetFields(fbeValue *Value) error {
    ...
    if err = fm.Vo.Set(fbeValue.Vo); err != nil {
        return err
    }
    if err = fm.Vo2.Set(fbeValue.Vo2); err != nil {
        return err
    }
    return err
}
