...
class FinalModelArrayExtra(buffer: fbe.Buffer, offset: Long, private val _size: Long) : fbe.FinalModel(buffer, offset)
{
    private val _model = FinalModelExtra(buffer, offset)

    // Get the allocation size
    fun fbeAllocationSize(values: Array<osa.Extra>): Long
    {
        var size: Long = 0
        var i: Long = 0
    ...
    fun fbeAllocationSize(values: java.util.ArrayList<osa.Extra>): Long
    {
        var size: Long = 0
        var i: Long = 0
        while ((i < values.size) && (i < _size))
    ...
    // Get the array
    fun get(size: fbe.Size): Array<osa.Extra>
    {
        val values = Array(_size.toInt()) { osa.Extra() }
    ...
    // Get the array
    fun get(values: Array<osa.Extra>): Long
    {
        assert((_buffer.offset + fbeOffset) <= _buffer.size) { "Model is broken!" }
        if ((_buffer.offset + fbeOffset) > _buffer.size)
            return 0
    ...
    fun get(values: java.util.ArrayList<osa.Extra>): Long
    {
        values.clear()
    ...
    // Set the array
    fun set(values: Array<osa.Extra>): Long
    {
        assert((_buffer.offset + fbeOffset) <= _buffer.size) { "Model is broken!" }
        if ((_buffer.offset + fbeOffset) > _buffer.size)
            return 0
    ...
    fun set(values: java.util.ArrayList<osa.Extra>): Long
    {
        assert((_buffer.offset + fbeOffset) <= _buffer.size) { "Model is broken!" }
        if ((_buffer.offset + fbeOffset) > _buffer.size)
            return 0
