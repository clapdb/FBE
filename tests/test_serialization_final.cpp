...
TEST_CASE("Serialization (Final): struct array", "[FBE][.]") {
  ...
  struct1.f3[0] = FBE::FastVec<uint8_t>(3, 48);
  struct1.f3[1] = FBE::FastVec<uint8_t>(3, 65);
  struct1.f4[0] = FBE::FastVec<uint8_t>(3, 97);
...
TEST_CASE("Serialization (Final): struct vector", "[FBE][.]") {
  ...
  struct1.f3.emplace_back(FBE::FastVec<uint8_t>(3, 48));
  struct1.f3.emplace_back(FBE::FastVec<uint8_t>(3, 65));
  struct1.f4.emplace_back(FBE::FastVec<uint8_t>(3, 97));
...
TEST_CASE("Serialization (Final): struct map", "[FBE][.]") {
  ...
  struct1.f3.emplace(10, FBE::FastVec<uint8_t>(3, 48));
  struct1.f3.emplace(20, FBE::FastVec<uint8_t>(3, 65));
  struct1.f4.emplace(10, FBE::FastVec<uint8_t>(3, 97));
...
// ============================================================================
// Final Mode Variant Tests
// ============================================================================

#include "../proto/variants_final_models.h"

TEST_CASE("Serialization (Final): variant monostate", "[FBE]") {
  ::variants::Value value;
  REQUIRE(value.v.index() == 0); // default is monostate

  FBE::variants::ValueFinalModel writer;
  size_t serialized = writer.serialize(value);
  REQUIRE(serialized == writer.buffer().size());
  REQUIRE(writer.verify());

  FBE::variants::ValueFinalModel reader;
  reader.attach(writer.buffer());
  REQUIRE(reader.verify());
...
TEST_CASE("Serialization (Final): variant string", "[FBE]") {
...
TEST_CASE("Serialization (Final): variant int32", "[FBE]") {
  ::variants::Value value;
...
TEST_CASE("Serialization (Final): variant double", "[FBE]") {
  ::variants::Value value;
...
TEST_CASE("Serialization (Final): variant struct", "[FBE]") {
...
TEST_CASE("Serialization (Final): variant vector of struct", "[FBE]") {
  FBE::FastVec<::variants::Simple> v;
...
TEST_CASE("Serialization (Final): variant vector of int32", "[FBE]") {
  FBE::FastVec<int32_t> v{1, 2, 3};
...
TEST_CASE("Serialization (Final): variant nested (Expr)", "[FBE]") {
  ::variants::Expr expr{FBE::FBEString("42")};
  ::variants::Value value;
  ...
  auto &expr_copy = std::get<::variants::Expr>(value_copy.v);
  REQUIRE(expr_copy.index() == 3);
  REQUIRE(std::get<3>(expr_copy) == "42");
  REQUIRE(value == value_copy);
}

TEST_CASE("Serialization (Final): variant hash with primitive and struct",
          "[FBE]") {
...
TEST_CASE("Serialization (Final): variant bytes[]", "[FBE]") {
  FBE::FastVec<uint8_t> v{65, 66, 67, 68, 69};
  FBE::FastVec<FBE::buffer_t> bytes_v;
...
TEST_CASE("Serialization (Final): variant string[]", "[FBE]") {
  FBE::FastVec<FBE::FBEString> string_v{"hello", "world"};
...
TEST_CASE("Serialization (Final): variant bytes{int32}", "[FBE]") {
...
TEST_CASE("Serialization (Final): variant bytes{string}", "[FBE]") {
