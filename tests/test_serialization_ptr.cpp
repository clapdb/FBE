...
TEST_CASE("Serialization (simple self-reference)", "[Ptr-based FBE]") {
  ...
  auto simplep = std::make_unique<::simple::Simple>(::simple::Simple(
      "single-info", nullptr, 2048,
      FastVec<std::unique_ptr<::simple::Simple>>(), FastVec<::simple::Simple>(),
      FBE::map<int32_t, std::unique_ptr<::simple::Simple>>(),
      FBE::map<int32_t, ::simple::Simple>()));
...
TEST_CASE("Serialization (extra circular-dependency)", "[Ptr-based FBE]") {
  ...
  ::extra::Info &&embedded_info3 = {
      ...
      std::make_unique<::extra::Extra>(
          ...
          FastVec<std::unique_ptr<::extra::Info>>(), std::list<::extra::Info>(),
  ...
  REQUIRE(copy.infopv[0]->extra->info2->info ==
          "pv-embedded-struct-info2-data");
...
TEST_CASE("Serilization (identical with templated-based code without ptr)",
          "[Ptr-based FBE]") {
  ::std::array<::osa::Extra, 1> osa_v = {
      ::osa::Extra(FBE::FBEString("extra"), "detail", ::osa::Sex::male,
                   ::osa::MyFLags::flag1)};
  ::osa::Simple osa = {
      "original",
      12,
  ...
  ::std::array<::sa::Extra, 1> sa_v = {::sa::Extra(FBE::FBEString("extra"),
                                                   "detail", ::sa::Sex::male,
                                                   ::sa::MyFLags::flag1)};
  ::sa::Simple sa = {
      "original",
...
TEST_CASE("Serialization (optional)", "[Ptr-based FBE]") {
  ...
  c1.extra = std::make_optional<::sa::Extra>(
      ::sa::Extra("extra", "detail", ::sa::Sex::female, ::sa::MyFLags::flag2));
...
TEST_CASE("Serialization (ptr-based import templated-based fbe)",
          "[Ptr-based FBE]") {
  ::osa::Extra extra(FBE::FBEString("extra"), "detail", ::osa::Sex::male,
                     ::osa::MyFLags::flag1);

  ::pkg::Info &&pkg_info = {"pkg_info", ::osa::Sex::male, ::osa::MyFLags::flag2,
                            std::move(extra)};
...
TEST_CASE("Serialization (variant)", "[Ptr-based FBE]") {
  ...
  SECTION("hash with primitive and struct") {
    FBE::HashMap<int32_t, ::variants_ptr::Simple> m;
  ...
  SECTION("variant used in container") {
    ...
    REQUIRE(
        std::holds_alternative<FBE::FBEString>(value_container_copy.vv.at(1)));
    ...
    REQUIRE(
        std::holds_alternative<FBE::FBEString>(value_container_copy.vm.at(2)));
  ...
  SECTION("variant of variant") {
    ...
    REQUIRE(std::get<::variants_ptr::Simple>(value_copy.vo2.value()).name ==
            "simple");
    REQUIRE(value == value_copy);
  }
}
...
TEST_CASE("Serialization (import template)", "[Ptr-based FBE]") {
  SECTION("variant") {
    ...
    REQUIRE(std::holds_alternative<FastVec<::variants::Simple>>(
        line_copy.vv.at(0)));
    REQUIRE(std::get<FastVec<::variants::Simple>>(line_copy.vv.at(0)).size() ==
            2);
    REQUIRE(
        std::get<FastVec<::variants::Simple>>(line_copy.vv.at(0)).at(0).name ==
        "simple1");
    REQUIRE(
        std::get<FastVec<::variants::Simple>>(line_copy.vv.at(0)).at(1).name ==
        "simple2");
    ...
    REQUIRE(
        std::holds_alternative<::variants::Simple>(line_copy.vm.at("key4")));
    REQUIRE(std::get<::variants::Simple>(line_copy.vm.at("key4")).name ==
            "simple");

    REQUIRE(line.vo.has_value());
    REQUIRE(std::holds_alternative<::variants::Simple>(line.vo.value()));
    REQUIRE(std::get<::variants::Simple>(line.vo.value()).name ==
            "optional simple");
  ...
  SECTION("enum and variant in a map") {
    ...
    REQUIRE(line_copy.vm.size() == 2);
    REQUIRE(std::holds_alternative<int32_t>(
        line_copy.vm.at(::enums::EnumInt8::ENUM_VALUE_1)));
    REQUIRE(std::get<int32_t>(
                line_copy.vm.at(::enums::EnumInt8::ENUM_VALUE_1)) == 42);
    REQUIRE(std::holds_alternative<::variants::Simple>(
        line_copy.vm.at(::enums::EnumInt8::ENUM_VALUE_2)));
    REQUIRE(std::get<::variants::Simple>(
                line_copy.vm.at(::enums::EnumInt8::ENUM_VALUE_2))
                .name == "simple");
    REQUIRE(line == line_copy);
  }
