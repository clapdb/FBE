...
public:
  ...
  GeneratorCpp &Ptr(bool ptr) noexcept {
    _ptr = ptr;
    return *this;
  }
  ...
  GeneratorCpp &Final(bool final) noexcept {
    _final = final;
    return *this;
  }
  ...
  GeneratorCpp &JSON(bool json) noexcept {
    _json = json;
    return *this;
  }
  ...
  GeneratorCpp &Proto(bool proto) noexcept {
    _proto = proto;
    return *this;
  }
  ...
  GeneratorCpp &Logging(bool logging) noexcept {
    _logging = logging;
    return *this;
  }
  ...
  GeneratorCpp &Arena(bool arena) noexcept {
    _arena = arena;
    return *this;
  }
  ...
  std::vector<std::string> ArenaTags() const noexcept {
    return {"ArenaManagedCreateOnlyTag"};
  }

  bool ImportPtr() const noexcept { return _import_ptr; }
  GeneratorCpp &ImportPtr(bool import_ptr) noexcept {
    _import_ptr = import_ptr;
    return *this;
  }
...
private:
  ...
  void GenerateFBE_Header(const fs::path &path);
  void GenerateFBE_Source(const fs::path &path);
  void GenerateFBEPtr_Header(const fs::path &path);
  void GenerateFBEModels_Header(const fs::path &path);
  void GenerateFBEModels_Inline(const fs::path &path);
  void GenerateFBEModels_Source(const fs::path &path);
  void GenerateFBECustomModels_Header(const fs::path &path);
  void GenerateFBECustomModels_Inline(const fs::path &path);
  void GenerateFBEFinalModels_Header(const fs::path &path);
  void GenerateFBEFinalModels_Inline(const fs::path &path);
  void GenerateFBEFinalModels_Source(const fs::path &path);
  void GenerateFBEProtocol_Header(const fs::path &path);
  void GenerateFBEProtocol_Source(const fs::path &path);
  void GenerateFBEJson_Header(const fs::path &path);
  ...
  void GeneratePackageProtocol_Header(const std::shared_ptr<Package> &p,
                                      bool final);
  void GeneratePackageProtocol_Source(const std::shared_ptr<Package> &p,
                                      bool final);
  void GenerateEnum(const std::shared_ptr<Package> &p,
  ...
  void GenerateEnumJson(const std::shared_ptr<Package> &p,
                        const std::shared_ptr<EnumType> &e);
  void GenerateEnumFieldModel(const std::shared_ptr<Package> &p,
                              const std::shared_ptr<EnumType> &e);
  void GenerateEnumFinalModel(const std::shared_ptr<Package> &p,
                              const std::shared_ptr<EnumType> &e);
  void GenerateFlags(const std::shared_ptr<Package> &p,
  ...
  void GenerateFlagsJson(const std::shared_ptr<Package> &p,
                         const std::shared_ptr<FlagsType> &f);
  void GenerateFlagsFieldModel(const std::shared_ptr<Package> &p,
                               const std::shared_ptr<FlagsType> &f);
  void GenerateFlagsFinalModel(const std::shared_ptr<Package> &p,
                               const std::shared_ptr<FlagsType> &f);
  void GeneratorStructForwardDeclaration(
      const std::vector<std::shared_ptr<StructType>> &structs);
  void GenerateVariantAlias(const std::shared_ptr<Package> &p,
                            const std::shared_ptr<VariantType> &v);
  void GenerateVariantIsEqualFunc(const std::shared_ptr<Package> &p,
                                  const std::shared_ptr<VariantType> &v);
  void GenerateVariantOutputStream(const std::shared_ptr<Package> &p,
                                   const std::shared_ptr<VariantType> &v);
  void GenerateStruct_Header(const std::shared_ptr<Package> &p,
                             const std::shared_ptr<StructType> &s);
  void GenerateStruct_Source(const std::shared_ptr<Package> &p,
                             const std::shared_ptr<StructType> &s);
  void GeneratePtrStruct_Header(const std::shared_ptr<Package> &p,
                                const std::shared_ptr<StructType> &s);
  void GeneratePtrStruct_Source(const std::shared_ptr<Package> &p,
                                const std::shared_ptr<StructType> &s);
  void GenerateStructOutputStream(const std::shared_ptr<Package> &p,
                                  const std::shared_ptr<StructType> &s);
  void GenerateStructLoggingStream(const std::shared_ptr<Package> &p,
                                   const std::shared_ptr<StructType> &s);
  void GenerateStructHash(const std::shared_ptr<Package> &p,
                          const std::shared_ptr<StructType> &s);
  void GenerateStructJson(const std::shared_ptr<Package> &p,
                          const std::shared_ptr<StructType> &s);
  void GenerateVariantFieldModel_Header(const std::shared_ptr<Package> &p,
                                        const std::shared_ptr<VariantType> &v);
  void GenerateVariantFieldModel_Source(const std::shared_ptr<Package> &p,
                                        const std::shared_ptr<VariantType> &v);
  void GenerateVariantFinalModel_Header(const std::shared_ptr<Package> &p,
                                        const std::shared_ptr<VariantType> &v);
  void GenerateVariantFinalModel_Source(const std::shared_ptr<Package> &p,
                                        const std::shared_ptr<VariantType> &v);
  void GenerateStructFieldPtrModel_Header(const std::shared_ptr<Package> &p,
                                          const std::shared_ptr<StructType> &s);
  void GenerateStructFieldModel_Header(const std::shared_ptr<Package> &p,
                                       const std::shared_ptr<StructType> &s);
  void GenerateStructFieldModel_Source(const std::shared_ptr<Package> &p,
                                       const std::shared_ptr<StructType> &s);
  void GenerateStructFieldPtrModel_Source(const std::shared_ptr<Package> &p,
                                          const std::shared_ptr<StructType> &s);
  void GeneratePtrStructFieldModel_Header(const std::shared_ptr<Package> &p,
                                          const std::shared_ptr<StructType> &s);
  void GeneratePtrStructFieldModel_Source(const std::shared_ptr<Package> &p,
                                          const std::shared_ptr<StructType> &s);
  void GenerateStructModel_Header(const std::shared_ptr<Package> &p,
                                  const std::shared_ptr<StructType> &s);
  void GenerateStructModel_Source(const std::shared_ptr<Package> &p,
                                  const std::shared_ptr<StructType> &s);
  void GeneratePtrStructModel_Header(const std::shared_ptr<Package> &p,
                                     const std::shared_ptr<StructType> &s);
  void GeneratePtrStructModel_Source(const std::shared_ptr<Package> &p,
                                     const std::shared_ptr<StructType> &s);
  void GenerateStructFinalModel_Header(const std::shared_ptr<Package> &p,
                                       const std::shared_ptr<StructType> &s);
  void GenerateStructFinalModel_Source(const std::shared_ptr<Package> &p,
                                       const std::shared_ptr<StructType> &s);
  void GenerateStructModelFinal_Header(const std::shared_ptr<Package> &p,
                                       const std::shared_ptr<StructType> &s);
  void GenerateStructModelFinal_Source(const std::shared_ptr<Package> &p,
                                       const std::shared_ptr<StructType> &s);
  ...
  // Check if a type is a fixed-size type that can be batch-processed
  // (primitive, enum, flags, uuid)
  bool IsFixedSizeType(const std::shared_ptr<Package> &p,
                       const std::string &type, bool optional) const;
  // Get the size of a fixed-size type, returns 0 for non-fixed types
  size_t GetFixedTypeSize(const std::shared_ptr<Package> &p,
                          const std::string &type) const;
  // Get the C++ type name for a fixed-size type (for unaligned_load/store)
  std::string GetFixedTypeCppName(const std::shared_ptr<Package> &p,
                                  const std::string &type) const;
  bool IsContainerType(const StructField &field);
  bool IsContainerType(const VariantValue &variant);
  bool IsStructType(const std::shared_ptr<Package> &p,
                    const std::string &field_type);
  bool IsCurrentPackageType(const std::string &field_type,
                            const std::string &delimiter = ".");
  bool IsPMRType(const std::shared_ptr<Package> &p,
                 const std::string &field_type);

  std::string ConvertVariantTypeName(const std::string &package,
                                     const VariantValue &variant);
  std::string ConvertVariantTypeNameAsArgument(const std::string &package,
                                               const VariantValue &variant);
  std::string ConvertVariantFinalModelType(const std::string &package,
                                           const VariantValue &variant);
  std::string
  ConvertPtrFieldModelType(const std::shared_ptr<Package> &p,
                           const std::shared_ptr<StructField> &field);
  std::string
  ConvertPtrVariantFieldModelType(const std::shared_ptr<Package> &p,
                                  const std::shared_ptr<VariantValue> &variant);
  std::string ConvertEnumType(const std::string &type);
  std::string ConvertTypeName(const std::string &package,
                              const std::string &type, bool optional);
  std::string ConvertTypeName(const std::string &package,
                              const StructField &field);
  std::string ConvertTypeNameAsArgument(const std::string &package,
                                        const StructField &field);
  std::string ConvertPtrTypeName(const std::string &package,
                                 const std::string &type);
  std::string ConvertPtrTypeName(const std::string &package,
                                 const std::string &type, bool optional,
                                 bool typeptr, bool as_argument);
  std::string ConvertPtrTypeName(const std::string &package,
                                 const StructField &field, bool as_argument);
  std::string ConvertPtrTypeNameAsArgument(const std::string &package,
                                           const StructField &field);
  std::string ConvertConstant(const std::string &type, const std::string &value,
                              bool optional);
  std::string ConvertConstantPrefix(const std::string &type);
  std::string ConvertConstantSuffix(const std::string &type);
  std::string ConvertDefault(const std::string &package,
                             const std::string &type);
  std::string ConvertDefault(const std::string &package,
                             const StructField &field);

  std::string ConvertOutputStreamType(const std::string &type,
                                      const std::string &name, bool ptr,
                                      bool optional);
  std::string ConvertOutputStreamValue(const std::string &type,
                                       const std::string &name, bool ptr,
                                       bool optional, bool separate);

  std::string ConvertLoggingStreamType(const std::string &type,
                                       const std::string &name, bool optional);
  std::string ConvertLoggingStreamValue(const std::string &type,
                                        const std::string &name, bool optional,
                                        bool separate);

  std::string ConvertNamespace(const std::string &package);
  std::string ConvertFileName(const std::string &package, FileType file_type,
                              bool is_header, bool is_ptr = false,
                              bool is_final = false);
};
